...
def test_start_task(client: TestClient):
    task_name = get_exist_task_name(client)
    task_original = get_task_by_name(client, task_name)
    response = client.post(f"/start_task?task_name={task_name}")
    task_started = get_task_by_name(client, task_name)
...
def test_toggle_task(client: TestClient):
    ...
    task_toggled = get_task_by_name(client, task_name)
    content = response.text
    assert response.status_code == 204
    assert content == ""
...
def test_update_task(client: TestClient):
    task_name = get_exist_task_name(client)
    original_task_setting = get_task_by_name(client, task_name)
    data = {
    ...
    task_setting = get_task_by_name(client, task_name)
